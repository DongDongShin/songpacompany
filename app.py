...
client = MongoClient('mongodb+srv://test:test@cluster0.0l29x.mongodb.net/?retryWrites=true&w=majority')
db = client.dbsparta
...
def delete_posts():
    ...
    try:
        ...
        db.posts.delete_one({'date': id})
        return jsonify({"result": "success", "msg": "해당 게시물을 삭제했습니다."})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))

@app.route("/modifying", methods=['POST'])
def modify_posts():
    token_receive = request.cookies.get('mytoken')
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
